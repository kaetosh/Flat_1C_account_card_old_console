...
class FileHandler:
    ...
    def _save_combined_results(df_upp: pd.DataFrame=pd.DataFrame(), df_non_upp: pd.DataFrame=pd.DataFrame()) -> None:
        ...
        # Записываем два DataFrame на разные листы
        with pd.ExcelWriter(temp_filename, engine='openpyxl') as writer:
            if not df_upp.empty:
                df_upp.to_excel(writer, sheet_name='UPP', index=False)
            if not df_non_upp.empty:
                df_non_upp.to_excel(writer, sheet_name='Non_UPP', index=False)
...
class UserInterface:
    ...
    def get_input() -> list[Path]:
        ...
        print("\nПеретащите один или несколько файлов Excel карточек 1С или папку с файлами и нажмите Enter:\n")
...
Если нужно обработать регистры по отдельности, перетаскивайте один или несколько файлов в окно программы.
В этом случае обработанные регистры будут расположены на отдельных листах excel-файла.
